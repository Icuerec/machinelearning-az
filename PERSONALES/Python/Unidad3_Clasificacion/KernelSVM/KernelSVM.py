# -*- coding: utf-8 -*-
"""
Created on Wed Aug  4 19:09:36 2021
...
df = pd.read_csv('Unidad3_Clasificacion/SVM/Social_Network_Ads.csv')
...
#classifier2 = SVC(kernel = 'poly',degree = 15)
...
plt.title('KernelSVC (Conjunto de Entrenamiento)')
plt.xlabel('Edad')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set.iloc[y_set == j, 0], X_set.iloc[y_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('KernelSVC (Conjunto de Test)')
plt.xlabel('Edad')
plt.ylabel('Sueldo Estimado')
plt.legend()
plt.show()
